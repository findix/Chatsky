...
function checkForm() {
    ...
    //密码password2验证
    if (form.password2 != undefined && !password.test(form.password2.value)) {
        //不符合匹配规则
        onClick($(form.password2)[0], '密码不规范，应为6~16位字母和数字');
        //阻止页面跳转
        return false;
    }
    ;
    ...
    //密码输入验证
    if (form.password && form.password2 && form.password.value != form.password2.value) {
        onClick($(form.password2)[0], '两次密码输入不同，请重新输入');
        //阻止页面跳转
        return false;
    }
}
...
$(document).ready(function () {
    ...
    $('#login_btn').click(function () {
        var $obj = $(this);
        $obj.text('登录中...');
        if (checkForm($('#login')[0])==undefined) {
            $.post("/login",
                {
    ...
    $('#register_btn').click(function () {
        var $obj = $(this);
        $obj.text('注册中...');
        if (checkForm($('#register')[0])==undefined) {
            $.post("/register",
                {
                ...
                function (data, status) {
                    if (data == "SUCCESS") {
                        ...
                        onClick($('#register [name=email]')[0], data);
                        $obj.text('注册');
                    }
                });
        }
